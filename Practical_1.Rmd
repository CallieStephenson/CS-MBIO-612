...
* Create a cell above the current one and describe the computation that you just did
  ...
  * Try to format (emphasize) the name `x` so it stands out.
  #I could not get the emphasize to work


```{r}
# Write you code in this cell
#This is a markdown test
#The variable y contains the mean of *x* which is made up of 5 values
x<-c(5:9)
y<-mean(x)
...
`c(1, 2, 3, "Hi") + 1 `
No, non-numeric argument to binary operator
* How about 
...
```{r}
# Write you code in this cell
x_axis <- seq(from=0, to =10, by = 0.5)
```
...
```{r}
# Write you code in this cell
#If you do not have qplot installed, run line 145 then
install.packages("tidyverse")
# add 'tidyverse' in packages by clicking install and finding the package
library(tidyverse)
x_axis <- seq(from=0, to =10, by = 0.5)
y_axis <- (x_axis^2+2*x_axis+3)
#run this code after you have run the previous code
qplot(x_axis, y_axis, xlab = "My x_axis", ylab = "My x_axis", main = "My amazing plot of x_axis versus y_axis", size=3)
#Note the directions label both axis 'x_axis'
```
