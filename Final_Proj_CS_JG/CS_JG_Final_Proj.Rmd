...
#
Biomass_for_Survey_Depth=data.frame(Site=NA,Total_Biomass=NA,Island=NA,Region=NA,Year=NA,Depth=NA)

for(i in 1:length(unique(raw_data$SITE))){
  Biomass_for_Survey_Depth=rbind(Biomass_for_Survey_Depth,c(unique(raw_data$SITE)[i], sum(raw_data$BiomassPerObs[which(raw_data$SITE==unique(raw_data$SITE)[i])]), raw_data$ISLAND[which(raw_data$SITE==unique(raw_data$SITE)[i])][1], raw_data$REGION[which(raw_data$SITE==unique(raw_data$SITE)[i])][1], raw_data$OBS_YEAR[which(raw_data$SITE==unique(raw_data$SITE)[i])][1], raw_data$DEPTH_M[which(raw_data$SITE==unique(raw_data$SITE)[i])][1]))
}
Biomass_for_Survey=Biomass_for_Survey[-1,]
Biomass_for_Survey$Total_Biomass <- as.numeric(Biomass_for_Survey$Total_Biomass)
#####
...
Shark_Biomass_for_Survey=data.frame(Site=NA,Total_Biomass=NA,Island=NA,Region=NA,Year=NA,Depth=NA)

for(i in 1:length(unique(SharkDF$SITE))){
  Shark_Biomass_for_Survey=rbind(Biomass_for_Survey,c(unique(SharkDF$SITE)[i], sum(SharkDF$BiomassPerObs[which(SharkDF$SITE==unique(SharkDF$SITE)[i])]), SharkDF$ISLAND[which(SharkDF$SITE==unique(SharkDF$SITE)[i])][1], SharkDF$REGION[which(SharkDF$SITE==unique(SharkDF$SITE)[i])][1], SharkDF$OBS_YEAR[which(SharkDF$SITE==unique(SharkDF$SITE)[i])][1], SharkDF$DEPTH_M[which(SharkDF$SITE==unique(SharkDF$SITE)[i])][1]))
}
Shark_Biomass_for_Survey=Shark_Biomass_for_Survey[-1,]
Shark_Biomass_for_Survey$Total_Biomass <- as.numeric(Shark_Biomass_for_Survey$Total_Biomass)
```
