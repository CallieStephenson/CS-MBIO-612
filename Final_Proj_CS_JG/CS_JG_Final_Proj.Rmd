...
To clean our data, we created two data frames based off the information we wanted to look at from the larger dataset. The code below makes one dataframe for Biomass found at each survey, and species richness of each survey. We also subsetted this data for shark-specific data analysis.
...
```{r cars}
### The dataset we used had some NA's in the depth column. If depth was going to be used in one of our analyses, we could remove the rows from the dataset that had NA's for depth by using the following code: 
