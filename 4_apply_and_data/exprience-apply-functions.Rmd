...
# use extra cells if needed
print(rnorm(5, 10, 1))
#can also work like rnorm(n=5, mean = 10, sd =1) if you want to write out the parameters instead of using default order
```
...
coral_cover <- mapply(generate_cover, coral_health_tbl$is_diseased)
...
```{r}
binary_health_status <- as.integer(coral_health_tbl$is_diseased)
...
```{r}
Q7_plot <- ggplot(coral_health_tbl, aes(x=coral_cover, y = stat(count / sum(count)), fill =is_diseased)) + geom_histogram(binwidth = 1, col=I("black")) + labs(x="Coral Cover", y="Normalized proportion")+ scale_fill_manual(values = c("white","grey"))
Q7_plot
...
#could also do
Q8_plot_2 <- ggplot(coral_health_tbl, aes(x=coral_cover, y = stat(count / sum(count)), fill =is_diseased)) + geom_density(adjust = 2)+ scale_fill_manual(values = c("white","grey"))+xlim(-10, 20)
Q8_plot
Q8_plot_2
```
### Q9
...
#there should be columns for location, genus name, species name, common name, and count
Genus <- c('Centropyge','Sphyraena','Centropyge','Sphyraena')
Species <- c('fisheri','barracuda', 'fisheri','barracuda')
Common_Name <- c('Fishers angelfish', 'Barracuda','Fishers angelfish', 'Barracuda')
Count <- c(23, 3, 14, 0)
Q9 <- data.frame(Genus = Genus, Species = Species, Common_Name = Common_Name, Count = Count) 
print(Q9)
...
#fix_missing_solar_R = function(x){if (x==NA) {replace Solar.R with mean from monthly_avg_SolarR} else{}}
airquality_no_NA_in_Solar_R <- apply(airq_tbl,1,fix_missing_solar_R)
```
